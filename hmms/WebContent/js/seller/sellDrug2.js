...
$(function() {
	...
	function loaddata() {
		...
		$.post(
						'/hmms/service/sellDrug/getSellDrugInfo',
						parm,
						function(result) {
							if (result.state == "0") {
								...
								layui.use('table',function() {
									...
									table.render({
										...
										cols : [ [
										          ...
										          {field : 'sellprice',title : '出售单价'},
										          {field : 'samount',title : '出售数量'},
										          {field : 'stime',title : '出售日期'},
	...
	layui.use('form', function() {
		...
		form.on('submit(formSubmit)', function(par) {
			...
			var data = par.field;
				...
				data.samount = $('#txtrAmount').val();
				data.sellprice = $('#txtInPrice').val();
				console.log(data);
				$.post('/hmms/service/sellDrug/addSellDrug2', data, function(result) {
					if (result.state == "0") {
